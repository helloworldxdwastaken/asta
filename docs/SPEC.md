...
- **Telegram bot** — `backend/app/channels/telegram_bot.py`: long polling when `TELEGRAM_BOT_TOKEN` set; same message handler as panel/WhatsApp. Built-in commands include `/status`, `/exec_mode`, `/allow`, `/allowlist`, `/approvals`, `/approve`, `/deny`, `/thinking`, `/reasoning`, `/subagents`.
...
- **Per-run overrides:** `sessions_spawn` accepts `model` and `thinking` (`off|low|medium|high`) and applies them to child runs (also reused by `sessions_send`).
