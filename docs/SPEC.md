...
- **Reasoning controls** — Per-user `thinking_level` (`off/minimal/low/medium/high/xhigh`) and `reasoning_mode` (`off/on/stream`) are stored in `user_settings`, exposed in Settings API/UI, and applied to provider calls/context instructions. Stream mode now runs through a dedicated OpenClaw-style stream state machine (`message_start/text_start/text_delta/text_end/message_end`) so assistant/reasoning updates stay monotonic across fallback/provider switches and tool-loop re-calls. Web chat supports SSE streaming via `POST /api/chat/stream` (`assistant`/`reasoning`/`status` events). Optional `final_mode` (`off|strict`) enforces visibility to `<final>...</final>` content only.
