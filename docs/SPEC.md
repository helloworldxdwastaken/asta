...
- **File management** — `backend/app/routers/files.py`: list/read under `ASTA_ALLOWED_PATHS` and workspace. **Virtual root**: "Asta knowledge" (`README.md`, `CHANGELOG.md`, and `docs/*.md`). **User context** = per-user `workspace/users/{user_id}/USER.md` (name, location, preferences); falls back to global `workspace/USER.md` for single-user mode.
...
- **Users (auth):** users (id UUID, username unique, password_hash bcrypt, role admin|user, created_at). JWT auth with 30-day expiry.
...
- **Per-user memories:** `workspace/users/{user_id}/USER.md` — structured markdown with Name, Location, About me, Preferences, Notes sections.
...
│   │   ├── main.py
│   │   ├── config.py
│   │   ├── auth_middleware.py  # JWT/Bearer auth middleware
│   │   ├── auth_utils.py      # JWT helpers, role checks
...
- **Previous (1.3.47):** Replaced native SwiftUI macOS app (`MACAPP/`) with cross-platform Tauri desktop app (`MACWinApp/`). Tauri v2 + React/TypeScript, builds for macOS (DMG) and Windows (MSI). All docs updated accordingly.
